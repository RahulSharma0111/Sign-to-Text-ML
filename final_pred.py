...
class Application:
    ...
    def __init__(self):
        ...
        if not self.vs.isOpened():
            print("Error: Could not open webcam with ID 0, trying ID 1...")
            self.vs = cv2.VideoCapture(1)
            
            if not self.vs.isOpened():
                print("Error: Could not open webcam with ID 1 either.")
        ...
        "By Team CodingWizards")
        self.root.protocol('WM_DELETE_WINDOW', self.destructor)
        self.root.geometry("1300x700")
        self.root.attributes('-fullscreen', True)
        self.root.configure(bg="#2e2e2e")

        # Set the panels with lighter shades
        self.panel = tk.Label(self.root, bg="#444444", fg="white")
        self.panel.place(x=100, y=3, width=650, height=640)

        self.panel2 = tk.Label(self.root, bg="#444444", fg="white")
        self.panel2.place(x=900, y=115, width=400, height=400)

        self.T = tk.Label(self.root, fg="white", bg="#2e2e2e")
        self.T.place(x=60, y=5)
        self.T.config(text="Sign Language To Text And Speech Conversion By Team CodingWizards", font=("Courier", 28, "bold"))

        self.panel3 = tk.Label(self.root, bg="#444444", fg="white")
        self.panel3.place(x=280, y=585)

        self.T1 = tk.Label(self.root, fg="white", bg="#2e2e2e")
        self.T1.place(x=10, y=580)
        self.T1.config(text="Character :", font=("Courier", 30, "bold"))

        self.panel5 = tk.Label(self.root, bg="#444444", fg="white")
        self.panel5.place(x=260, y=632)

        self.T3 = tk.Label(self.root, fg="white", bg="#444444")
        self.T3.place(x=10, y=632)
        self.T3.config(text="Sentence :", font=("Courier", 30, "bold"))

        self.T4 = tk.Label(self.root, fg="red", bg="#444444")
        self.T4.place(x=10, y=680)
        self.T4.config(text="Suggestions :", font=("Courier", 30, "bold"))

        # Adjusting button positions to prevent overlap
        self.b1 = tk.Button(self.root, bg="#555555", fg="white")
        self.b1.place(x=390, y=750)

        self.b2 = tk.Button(self.root, bg="#555555", fg="white")
        self.b2.place(x=590, y=750)

        self.b3 = tk.Button(self.root, bg="#555555", fg="white")
        self.b3.place(x=790, y=750)

        self.b4 = tk.Button(self.root, bg="#555555", fg="white")
        self.b4.place(x=990, y=750)

        #speak button
        self.speak = ctk.CTkButton(self.root, text="Speak", font=("Courier", 20),width=120, height=40, corner_radius=20,fg_color="#555555", text_color="white",command=self.speak_fun)
        self.speak.place(x=1405, y=650)

        #clear button
        self.clear = ctk.CTkButton(self.root, text="Clear", font=("Courier", 20),width=120, height=40, corner_radius=20,fg_color="#555555", text_color="white",command=self.clear_fun)
        self.clear.place(x=1270, y=650)

        #translate button
        self.translate = ctk.CTkButton(self.root, text="Translate", font=("Courier", 20),
                               width=120, height=40, corner_radius=20,
                               fg_color="#555555", text_color="white", command=self.translate_fun)
        self.translate.place(x=1040, y=650)
    ...
    def video_loop(self):
        try:
            ...
            if hands[0]:
                ...
                if y - offset < 0 or x - offset < 0 or y + h + offset >= cv2image_copy.shape[0] or x + w + offset >= cv2image_copy.shape[1]:
                    # Bounding box out of image bounds, skip this frame
                    self.root.after(30, self.video_loop)
                    return
                ...
                if white is None:
                    print("Error: Could not load white.jpg file. Make sure it exists in the same directory.")
                    self.root.after(30, self.video_loop)
                    return
                    
                if image.size > 0:  # Check if image is valid
            ...
            print("<attribute '__traceback__' of 'BaseException' objects>")
            print(traceback.format_exc())
        
        # Schedule the next video frame capture
        self.root.after(30, self.video_loop)
    ...
    def translate_fun(self):
        if self.str.strip() != "":
            try:
                translation = self.translator.translate(self.str, dest='hi') 
                translated_text = translation.text
                tk.messagebox.showinfo("Translation", f"Translated to Hindi:\n{translated_text}")
            except Exception as e:
                tk.messagebox.showerror("Error", f"Application under construction😊")
        else:
            tk.messagebox.showwarning("Warning", "No text to translate.")
    ...
    def predict(self, test_image):
        ...
        if len(self.str.strip())!=0:
            ...
            if len(word.strip())!=0:
                pred.check(word)
                lenn = len(pred.suggest(word))
                if lenn >= 4:
                    self.word4 = pred.suggest(word)[3]

                if lenn >= 3:
                    self.word3 = pred.suggest(word)[2]

                if lenn >= 2:
                    self.word2 = pred.suggest(word)[1]

                if lenn >= 1:
                    self.word1 = pred.suggest(word)[0]
            else:
                self.word1 = " "
                self.word2 = " "
                self.word3 = " "
